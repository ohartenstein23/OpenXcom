...
ConfirmDestinationState::ConfirmDestinationState(Craft *craft, Target *target) : _craft(craft), _target(target)
{
	...
	int btnOkX = transferAvailable ? 33 : 68;
	int btnCancelX = transferAvailable ? 173 : 138;

	// Create objects
	_window = new Window(this, 244, 72, 6, 64);
	_btnOk = new TextButton(50, 12, btnOkX, 104);
	_btnTransfer = new TextButton(50, 12, 103, 104);
	...
	_btnTransfer->setText(tr("STR_TRANSFER"));
...
void ConfirmDestinationState::btnTransferClick(Action *)
{
	...
	else if (Options::storageLimitsEnforced && targetBase->storesOverfull(_craft->getItems()->getTotalSize(_game->getMod())))
	{
